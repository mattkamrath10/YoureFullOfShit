...
## Push notifications
Web Push is implemented for admins (`docs/ADMIN_PUSH_IPHONE.md`). Email via Resend remains the fallback when a device has no working subscription.
...
3. In Render (and `.env.local`), set **server-only** vars (never `NEXT_PUBLIC_`):
   - `RESEND_API_KEY`
   - `RESEND_FROM_EMAIL` (e.g. `Last Storyteller <mattk@laststoryteller.com>`)
   - `SUPABASE_SERVICE_ROLE_KEY` (Supabase → Project Settings → API → service_role)
   - Optional fallback: `ADMIN_NOTIFY_EMAIL` (comma-separated) if Auth email lookup fails
4. Set `NEXT_PUBLIC_SITE_URL` to `https://laststoryteller.com` so Review links are correct.
...
2. Check Resend dashboard / inbox for “New Story Submitted — Last Storyteller”.
