...
Last Storyteller uses Web Push for new pending-story notifications. On iPhone, Web Push is
available in Safari on iOS 16.4 or later only after Last Storyteller is installed as a Home
Screen web app.

If you previously installed the old YFOS icon, delete it and add Last Storyteller again so
the new icon and name are used.
...
VAPID_SUBJECT=mailto:mattk@laststoryteller.com
```
...
If VAPID keys already exist, keep them and only update `VAPID_SUBJECT`. Generating
new keys requires every admin device to enable phone alerts again.

## Install and enable on iPhone

1. Update the iPhone to iOS 16.4 or newer.
2. In Safari, visit `https://laststoryteller.com` (or the Render hostname until DNS is live).
3. Tap **Share**, choose **Add to Home Screen**, then tap **Add**.
4. Open Last Storyteller using the new Home Screen icon (not the Safari tab).
...
- If permission was denied, re-enable notifications for the installed Last Storyteller web
